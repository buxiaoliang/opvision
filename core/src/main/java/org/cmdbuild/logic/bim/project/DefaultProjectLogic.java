...
public class DefaultProjectLogic implements ProjectLogic {
	...
	public Project createProject(final Project project) {
		...
		final BimFacadeProject baseProject = bimServiceFacade.createProject(bimProject);
	...
	public void updateProject(final Project project) {
		final BimFacadeProject inputProject = LOGIC_TO_BIM_PROJECT.apply(project);
		final BimFacadeProject updatedProject = bimServiceFacade.updateProject(inputProject);
	...
	public DataHandler downloadIfc(final String projectId) {
		return bimServiceFacade.downloadIfc(projectId);
	}
	...
	private static final Function<Project, PersistenceProject> LOGIC_TO_PERSISTENCE_PROJECT = new Function<Project, PersistenceProject>() {
		...
		public PersistenceProject apply(final Project input) {
			...
			cmProject.setImportMapping(input.getImportMapping());
			return cmProject;
		}
	};

	@Override
	public String downloadMapping(Project project) {
		final PersistenceProject persistenceProject = LOGIC_TO_PERSISTENCE_PROJECT.apply(project);
		return bimPersistence.getMapping(persistenceProject);
	}
