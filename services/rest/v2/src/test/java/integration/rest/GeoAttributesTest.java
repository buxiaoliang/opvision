...
public class GeoAttributesTest {

	@ClassRule
	public static ServerResource<GeoAttributes> server = ServerResource.newInstance(GeoAttributes.class) //
	...
	public void setUp() throws Exception {
		server.service(geoAttributes = mock(GeoAttributes.class));
	...
	public void getAllAttributes() throws Exception {
		...
		final ResponseMultiple<Attribute2> expectedResponse = newResponseMultiple(Attribute2.class) //
				...
				.withMetadata(newMetadata() //
						...
						.build()) //
				.build();
		when(geoAttributes.readAllAttributes(anyString(), anyInt(), anyInt(), anyBoolean())) //
		...
		verify(geoAttributes).readAllAttributes(eq("foo"), eq(456), eq(123), eq(true));
	}
	...
	public void getAttribute() throws Exception {
		...
		final ResponseSingle<Attribute2> expectedResponse = newResponseSingle(Attribute2.class) //
				...
				.withMetadata(newMetadata() //
						...
						.build()) //
				.build();
		when(geoAttributes.readAttribute(anyString(), anyString())) //
				.thenReturn(expectedResponse);
		...
		verify(geoAttributes).readAttribute(eq("foo"), eq("bar"));
	}
