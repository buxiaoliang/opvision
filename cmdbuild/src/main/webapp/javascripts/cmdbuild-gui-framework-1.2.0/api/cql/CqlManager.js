...
function cqlEvaluate(obj, callback, callbackScope) {
	...
	var treeObj = new treeObject(configurator, cqlManager.lexer.tree, meta,
			obj.form);
	treeObj.resolve(false, function(response) {
		response = configurator.cleanString(response);
...
function CqlManager(configurator) {
		...
		for (var i = 0; i < attributes.length; i++) {
			...
			if (attribute.filter) {
				if (typeof attribute.filter === "string") {
					attribute.filter = JSON.parse(attribute.filter);
				}
				if (attribute.filter.text || attribute.filter.expression) {
	...
	this.compileAttribute = function(form, attribute) {
		var filterExpression = attribute.filter.text;
		if (!filterExpression) {
			filterExpression = attribute.filter.expression;
		}
		if (!(attribute.filter && filterExpression)) {
			return;
		}
		this.lexer.analize(filterExpression);
...
function lexer(configurator) {
	...
	this.analize = function(str) {
		...
		this.trimFirstSpaceOnTree(this.tree);
	...
	this.trimFirstSpaceOnTree = function(tree) {
		if (tree.length > 0 && typeof tree[0] == "object" && tree[0].type == "text") {
			var app = tree[0].value;
			tree[0].value = tree[0].value.substr(1);
		}
	};
...
function variablesTable(configurator) {
	...
	this.getField = function(table, form, field) {
		for (i = 0; i < table.length; i++) {
			var row = table[i];
			if (row.form == form && row.field == field) {
				return row;
			}
		}
		return null;
	...
	this.generateVariables = function(form, field, tree, meta) {
		for (var i = 0; i < tree.length; i++) {
			if (tree[i].type == undefined) { // is an array
				...
				var tableField = this.getField(this.table[form], form, field);
//				if (tableField === null) {
...
/*				} else {
					tableField.manager = tree[i].manager;
					tableField.variable = variables[0];
				}
*/			} else {
				;// nop (is a constant)
			}
...
function commandsTable(configurator) {
	...
	this.resolve = function(form, field, callback, callbackScope) {
		if (this.getTree(form, field)) {
			this.getTree(form, field).resolve(false, function(response) {
				callback.apply(callbackScope, [response]);
			}, this);
		} else {
...
function treeObject(configurator, tree, meta, form) {
	...
	this.resolve = function(withQuotes, callback, callbackScope) {
		...
		try {
			...
			this.resolveTree(temporaryTree, "", withQuotes, function(response) {
				callback.apply(callbackScope, [response]);
			}, this);
		} catch (e) {
			callback.apply(callbackScope, [undefined]);//to be controlled
		}
	};
	this.quote = function(str, withQuotes) {
		var QUOTE = (withQuotes) ? "CMDBUILD_QUOTE" : "";
		return QUOTE + str + QUOTE;
	};
	this.resolveTree = function(tree, returnValue, withQuotes, callback, callbackScope) {
		var QUOTE = "CMDBUILD_QUOTE";
		if (tree.length <= 0) {
		...
		var node = tree[0];
		tree.splice(0, 1);
		if (node.manager && this.configurator.manager[node.manager].isQuotedManager === true) {
			withQuotes = true;
		}
		if (node.type == undefined) { // is an array
			var temporaryTree = node.slice();
			this.resolveTree(temporaryTree, "", withQuotes, function(response) {
				this.resolveTree(tree, returnValue + response, withQuotes, callback,
						callbackScope);
			}, this);
			...
			if (this.isInMyMeta(arValues[0])) {// tree[i].value)) {
				this.resolveMeta(arValues[0], withQuotes, function(response) {
					if (response == undefined) {
						response = $.Cmdbuild.global.CQLUNDEFINEDVALUE;
						this.resolveTree(tree, returnValue + response, withQuotes,
					...
					this.evaluate(node.manager, this.form, response, function(
						...
						this.resolveTree(tree, returnValue + this.quote(evaluated, withQuotes), false,
								callback, callbackScope);
					}, this);
				}, this);
			} else {
				...
				this.evaluate(node.manager, this.form, strValue, function(
					...
					this.resolveTree(tree, returnValue + this.quote(evaluated, withQuotes), false, callback,
							callbackScope);
				}, this);
			}
		} else {
			this.resolveTree(tree, returnValue + node.value, withQuotes, callback,
					callbackScope);
		}
	};
	...
	this.resolveMeta = function(key, withQuotes, callback, callbackScope) {
		// var meta = this.commandTable.getMeta(this.form, this.field);
		this.meta[key].resolve(withQuotes, function(response) {
			callback.apply(callbackScope, [response]);
		}, this);
	};
