function Configurator(output) {
	...
	this.manager = {
		...
		cql: {
			...
			getValue: function(form, filter, callback, callbackScope) {
				...
				$.Cmdbuild.utilities.proxy.getCqlResult(config, callObj, this);
			}
		},
		js: {
			isQuotedManager: true,
			isVariableManager: false,
			getValue: function(form, name, callback, callbackScope) {
			...
			safeJSEval: function(stringTOEvaluate) {
				...
				stringTOEvaluate = cleanString(stringTOEvaluate);
				var resultOfEval = "";
				output({
					type: "debug",
	...
	this.cleanString = function(value) {
	...
	function cleanString(value) {
		...
		var ret = value.replace(regexpr, "'");

		regexpr = new RegExp(QUOTE + "'", "g");
		ret = ret.replace(regexpr, "'");
