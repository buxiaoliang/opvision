	...
	var form = function() {
		...
		this.show = function() {
			...
			$.each(attributes, function(index, attribute) {
				...
				// readonly forms
				if (me.isReadOnlyForm()) {
					attribute.writable = false;
				}

				if (attribute.writable && (value == null || value == undefined)){
		 ...
		 */
		this.change = function(param, callback, callbackScope) {
			function widgetsDataCB () {
				// validate form
				var errors = [];
				var $form = $("#" + this.config.form);
				var attributes = this.getBackendAttributes();
				var widgets_errors = $.Cmdbuild.widgets.getWidgetsErrors(
						this.config.form, this.getBackendWidgets());
				// validate form
				if (!$form.valid() || widgets_errors.length) {
					// attributes
					...
					// widgets 
					$.each(widgets_errors, function(index, error) {
						errors.push({
							field : "Widget " + error.label,
							message : error.error
						});
					});
				} else {
				...
				callback.apply(callbackScope, [errors]);
			}

			// load widgets data
			var backend = this.getBackend();
			if (backend.getWidgets) {
				$.Cmdbuild.widgets.getWidgetsData(this.config.form, backend.getWidgets(), widgetsDataCB, this);
			} else {
				widgetsDataCB.apply(this);
			}
		};
		 ...
		 * Is read-only form
		 */
		this.isReadOnlyForm = function() {
			if (this.getBackend().isReadOnly && this.getBackend().isReadOnly()) {
				return true;
			}
			return this.config.readonly && this.config.readonly == "true";
		};
