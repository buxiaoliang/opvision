	...
	var commands = {
		...
		advance: function(param) {
			try {
				...
				// on flush success function
				function onFlushSuccess(response) {
					if (param.dialog) {
				...
				// change
				$.Cmdbuild.dataModel.change(param, {
					success : function() {
						// flush
						$.Cmdbuild.dataModel.flush(param, onFlushSuccess);
					},
					scope : this
				});
			} catch (e) {
		...
		save: function(param) {
			try {
				...
				$.Cmdbuild.dataModel.change(param, {
					success: function() {
						$.Cmdbuild.dataModel.flush(param, flushCB);
		...
		refreshGrid : function(params, callback, callbackScope) {
			// set form pagination
			var grid = $.Cmdbuild.dataModel.forms[params.form];
		...
		/**
		 * @param {Object} params
		 * @param {String} params.id Report id is alternative to Report name
		 * @param {String} params.name Report name is alternative to Report id
		 * @param {String} param.extension Report extension. Optional.
		 * @param {Object} param.customattrs Attributes customizations. Optional.
		 */
		openreport : function(params) {
			var form = new $.Cmdbuild.standard.openreport();
			form.init(params);
		}
	};
