	...
	var grid = function() {
		...
		this.onBackendLoaded = function() {
			...
			this.paginationSettings = {
					...
					fnRowCallback : param.fnRowCallback ? param.fnRowCallback : null,
		...
		this.chargeFilter = function() {
			...
			param.grid = this;
			if (param.cqlRefForm && param.cqlRefField) {
				$.Cmdbuild.CqlManager.resolve(param.cqlRefForm, param.cqlRefField, this.show, this);
			}
			else {
				this.show("");
			}
		...
		this.show = function(filterCql) {
			...
			try {
				...
				var table= $('#' + this.id).dataTable({
					...
					"fnRowCallback": me.paginationSettings.fnRowCallback ? $.Cmdbuild.dataModel.resolveVariable(me.paginationSettings.fnRowCallback) : null,
...
//					var data = table.row(row).data();
		...
		this.drawCallback = function(table, grid, param, settings) {
	        ...
	        api.column(0, {page:'current'} ).data().each( function ( group, i ) {
	            if ( last !== group ) {
	                $(rows).eq( i ).before(
	                    '<tr class="cmdbuildGroupByHeader"><td colspan="6">'+group+'</td></tr>'
	                );
			...
			for (var i = 0; attributes && i < attributes.length; i++) {
				...
				if (attribute.displayableInList === true || attribute.displayableInList === "true") {
					if (attribute.type == "IMAGEBUTTON") {
						...
						var text = attribute.text ? attribute.text : "";
						columnDefs.push({
							"width" : attribute.text ? null : 20,
							"targets" : index,
							"data" : null,
							"title" : "&nbsp;",
							"orderable" : false,
							"defaultContent" : "<span " + strTooltip
									+ " class='" + attribute.className
									+ "' method='" + attribute.onClick + "'>"
									+ (attribute.text ? attribute.text : "") + "</span>"
						});
					}
		...
		this.showAttributes = function(param) {
			try {
				...
				// add row buttons
				if (param.rowButtons) {
					$.Cmdbuild.elementsManager.loadIconButtons(attributes, param.rowButtons, param.positionButtons);
				}

				// add checkbox
				if (param.selection === "true") {
					$.Cmdbuild.elementsManager.pushSelectionCheck(attributes, "left");
				}
		...
		this.getFieldValue = function(card, attribute) {
			if (! card[attribute.name]) {
				...
				return $("<div></div>").html(card[attribute.name]).text();
			}
			else {
