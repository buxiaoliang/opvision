(function($) {
	...
	var fieldsManager = {
		 ...
		 */
		generateHTML : function(attribute, formName, formContainer, value, cqlRefForm) {
			...
			var $field = this.getFormField(attribute, formName, formContainer, value, cqlRefForm);
			if (attribute.hidden) {
		 ...
		 * @return {jQuery}
		 */
		getFormField : function(attribute, formName, formContainer, value, cqlRefForm) {
			var me = this;
			var $output;
			if (attribute.hidden) {
				...
				case me.types.REFERENCE:
					$output = getReferenceField(formName, formContainer, attribute, value, cqlRefForm);
					break;
				case me.types.SELECT:
					$output = getSelectField(formName, attribute, value);
					break;
	...
	var getBooleanField = function(form, attribute, value) {
		...
		if (attribute.writable === true || attribute.writable === "true") {
			...
			var $text = (value === true || value === "true") ? 
					$.Cmdbuild.translations.getTranslation("cgf_core_boolfield_yes", "Yes") : 
						$.Cmdbuild.translations.getTranslation("cgf_core_boolfield_no", "No");
			return [$text, $input];
		}
	};
	...
	var getDateTimeField = function(form, attribute, value) {
		if (attribute.writable === true || attribute.writable === "true") {
			...
			var $input = getHiddenField(form, attribute._id, value);
			var $text = $("<span></span>").text(value ? $.Cmdbuild.utilities.convertDateDB2GUI(value, attribute.type) : "");
			return [$text, $input];
		}
	};
		...
		if (writable) {
			...
			$("#" + form).on('DOMNodeInserted', function(event) {
				if ($("#" + form).find("#" + $input.attr("id")).length
						&& ($(event.target).attr("id") === $input.attr("id") 
								|| $(event.target).find("#" + $input.attr("id")).length)) {
					$input.selectmenu({ 
	 ...
	 * @return {jQuery}
	 */
	var getReferenceField = function(form, formContainer, attribute, value, cqlRefForm) {
		var $input;
		var output = [];
		var writable = attribute.writable === true || attribute.writable === "true";
		if (writable) {
		...
		// target information
		var targetType, targetName;
		if (attribute.target) {
			targetType = attribute.target.type;
			targetName = attribute.target.name;
		} else {
			targetType = attribute.targetType;
			targetName = attribute.targetClass;
		}

		// preselect information
		var preselectIfUnique = false;
		if (attribute.metadata
				&& attribute.metadata["system.type.reference.preselectIfUnique"]) {
			preselectIfUnique = attribute.metadata["system.type.reference.preselectIfUnique"] === true
					|| attribute.metadata["system.type.reference.preselectIfUnique"] === "true";
		}
		...
		new $.Cmdbuild.standard.referenceField({
			...
			readOnly : !writable,
			optionsLimit : attribute.optionsLimit,
			value : value,
			displayAttribute : attribute.displayAttribute,
			mandatory : attribute.mandatory,
			preselectIfUnique : preselectIfUnique,
			cqlRefForm : cqlRefForm,
			cqlRefField : attribute.name,
			customBackend : attribute.customBackend,
			sortAttribute : attribute.sortAttribute,
	 ...
	 * @return {jQuery}
	 */
	var getSelectField = function(form, attribute, value) {
		var $input;
		var writable = attribute.writable === true || attribute.writable === "true";
		if (writable) {
			var $input = $("<select></select>");
		...
		var id = $input.attr("id");
		new $.Cmdbuild.standard.selectField({
			form : form,
			id : id,
			readOnly : !writable,
			...
			value : value,
			hideEmptyOption : attribute.hideEmptyOption,
			refreshGridAfterInit : attribute.refreshGridAfterInit
		});
		return $input;
	};
	...
	var getTextField = function(form, attribute, value) {
		...
		if (attribute.writable === true || attribute.writable === "true") {
			...
			if (! isHtml) {
				value = $.Cmdbuild.utilities.plain2html(value);
			}
			$text.html(value);
