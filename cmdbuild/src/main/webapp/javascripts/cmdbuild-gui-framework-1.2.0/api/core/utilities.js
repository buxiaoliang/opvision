...
*/				callback.apply(callbackScope, [callbackParams]);
		...
		convertDateGUI2DB: function(entryValue, dateType) {
			if (dateType == "date" && entryValue) {
				...
				if (ar.length == 3) {
					...
					return "";
			} else if (dateType == "time" && entryValue) {
				return "1970-01-01T"+ entryValue;
			} else if (dateType == "dateTime" && entryValue) {
		...
		startApplicationCallBack: function() {
			...
			var htmlStr = $.Cmdbuild.elementsManager.insertChildren(root);
		...
		getButtonsFromXML : function(xmlElement) {
			var htmlStr = "";
			var buttonsContainer = $.grep(xmlElement.childNodes, function(element, index) {
				return element.nodeName === "buttons";
			});

			if (buttonsContainer.length) {
				var buttons = $.grep(buttonsContainer[0].childNodes, function(element, index) {
				...
				$.each(buttons, function(index, button) {
					htmlStr += $.Cmdbuild.elementsManager.toHtml(button);
				});
		 ...
		 */
		popupMessage : function(title, message, id, width, height, onclose) {
			$('<div />').attr("id", id).html(message).dialog({
				title : title,
		 ...
		 * @param {String} id
		 * @param {Object} btn1 First button definition
		 * @param {String} btn1.text First button label
		 * @param {Function} btn1.click Fist button action
		 * @param {Object} btn2 Second button definition
		 * @param {String} btn2.text Second button label
		 * @param {Function} btn2.click Second button action
		 * @param {Integer} width
		 * @param {Integer} height
		 */
		showConfirmPopup : function(title, message, id, apply, cancel, width, height) {
			$('<div />').attr("id", id).html(message).dialog({
				title : title,
				dialogClass : $.Cmdbuild.global.getThemeCSSClass(),
				closeText: $.Cmdbuild.translations.getTranslation("cgf_core_dialog_close", "Close"),
		...
		getFormValidatorObject : function() {
			return {
				onsubmit : false,
				ignore : "input:hidden",
				rules : {},
		 ...
		 */
		plain2html : function(str) {
			// before of all encode string
			var encstr = $.Cmdbuild.utilities.htmlencode(str);
		 ...
		 * @return {String}
		 */
		getClassDescription : function (classId) {
			var classes = $.grep($.Cmdbuild.dataModel.classes, function(c) {
				return c._id == classId;
			});
			if (classes.length) {
				return classes[0].description;
			} else {
				return classId;
			}
		},
		 ...
		 * @return {String}
		 */
		getProcessDescription : function (processId) {
			var processes = $.grep($.Cmdbuild.dataModel.processes, function(p) {
				return p._id == processId;
			});
			if (processes.length) {
				return processes[0].description;
			} else {
				return processId;
			}
		},
		 ...
		 * @return {Object}
		 */
		getReportsOverrides : function(xml) {
			var obj = {};
			var $xml = $(xml);
			var $reports = $xml.find("reports");

			$.each($reports.children("report"), function(rindex, report) {
				var $report = $(report);
				obj[$report.attr("name")] = {
					extension : $.Cmdbuild.dataModel.resolveVariable($report.attr("extension")),
					attributes : {}
				};
				var attributes = obj[$report.attr("name")].attributes;
				$.each($report.children("attribute"), function(aindex, attribute) {
					var $attribute = $(attribute);
					var properties = attributes[$attribute.attr("name")] = {};
					$.each($attribute.children("property"), function(pindex, property) {
						var property_name = property.getAttribute("name")
						var property_value = property.getAttribute("value")
						properties[property_name] = $.Cmdbuild.dataModel.resolveVariable(property_value);
					});
				});
			});
