...
//						}
		...
		getData: function(form) {
			if (! ($.Cmdbuild.dataModel.model[form] && $.Cmdbuild.dataModel.model[form].data !== undefined)) {
				return [];
			}
		...
		updateData: function(form, data) {
			if ($.Cmdbuild.dataModel.model[form]) {
				$.Cmdbuild.dataModel.model[form].data = data;
			}
		},
		getMetaData: function(form, key) {
			if ($.Cmdbuild.dataModel.model[form]) {
				return $.Cmdbuild.dataModel.model[form].metadata[key];
			}
			return undefined;
		},
		updateMetaData: function(form, key, value) {
			if ($.Cmdbuild.dataModel.model[form]) {
				$.Cmdbuild.dataModel.model[form].metadata[key] = value;
			}
		},
		...
		/**
		 * Push form data into store
		 * @param {Object} param
		 * @param {String} param.form
		 * @param {String} param.type
		 * @param {Integer} param.currentIndex
		 * @param {Object|Array} param.data
		...
		push: function(param) {
			try {
				$.Cmdbuild.dataModel.model[param.form] = {
					...
					metadata : param.metadata ? param.metadata : {}
				};
			}
			catch (e) {
		 ...
		 * @param {DOM} callback.scope
		 */
		change: function(param, callback) {
			// Raise error if has not callback. Used to notice the user when is
			// using change method in old mode.
			if (callback === undefined) {
				$.Cmdbuild.errorsManager
						.popup({
							name : "dataModel.change",
							message : "$.Cmdbuild.dataModel.change has been changed. Please check custom code or contact system administrator!"
						});
				return;
			}
			...
			try {
				param.data = $.Cmdbuild.dataModel.model[param.form].data;
				param.formObject.change(param, function(errors) {
					if (!errors.length) {
						if (callback.success) {
						...
						if (callback.failure) {
							callback.failure.apply(callback.scope, [errors]);
						}
					}
		...
		resolveCMDBuildFunctions: function(field) {
			return $.Cmdbuild.customfunctions[field].apply();
		},
			...
			if (("" + variable).substr(0, 4) == "$and") {
 				...
 				ret = $.Cmdbuild.dataModel.resolveNot(variable.substr(4));
  	 		    ...
  	 		    if (arStr.length == 1) {
       	 		...
       	 		else if (arStr[0] == "cmdbuildfunctions"){
   	 				ret = $.Cmdbuild.dataModel.resolveCMDBuildFunctions(arStr[1]);
       	 		}
       	 		else {
