(function () {
	...
	CMDBuild.clearComponent = function(component) {
	    if (component.items) {
		component.items.each(function(childItem) { 
		    CMDBuild.clearComponent(childItem);
		    var store;
		    if (childItem.getStore) {
			store = childItem.getStore();
			store.clearData();
		    }
		    if (childItem.destroy) {
			    childItem.destroy();
		    }
		    component.remove(childItem);
		}, this);
	    }
