(function () {
	...
	Ext.define('CMDBuild.core.cache.Cache', {
		...
		managedGroupsArray: [
			...
			CMDBuild.core.constants.Proxy.CARD,
			...
			CMDBuild.core.constants.Proxy.GROUP_PRIVILEGE,
			CMDBuild.core.constants.Proxy.LOOKUP,
			CMDBuild.core.constants.Proxy.USER,
			CMDBuild.core.constants.Proxy.WIDGET,
			CMDBuild.core.constants.Proxy.WORKFLOW
		],

		/**
		 * Managed group that have a more short expiring time
		 *
		 * @cfg {Array}
		 *
		 * @private
		 ...
		 * @private
		 */
		isCacheable: function (groupId) {
			if (
				!Ext.isEmpty(groupId)
				&& groupId != CMDBuild.core.constants.Proxy.UNCACHED // Force uncached AJAX calls
			) {
				return Ext.Array.contains(this.managedGroupsArray, groupId);
			}
		 ...
		 * @private
		 */
		isCacheableEntity: function (groupId) {
			if (
				!Ext.isEmpty(groupId)
				&& groupId != CMDBuild.core.constants.Proxy.UNCACHED // Force uncached AJAX calls
			) {
				return Ext.Array.contains(this.managedEntities, groupId);
			}
		...
		isExpired: function (parameters) {
			...
			var isExpired = true;
			var entityType = this.isCacheableEntity(parameters[CMDBuild.core.constants.Proxy.GROUP_ID]);
			var timeForEntity = (entityType) ? CMDBuild.core.configurations.Timeout.getCacheEntity() : CMDBuild.core.configurations.Timeout.getCache();
			if (this.cachedDataExists(parameters)) {
				...
				isExpired = (
					...
					|| cachedObject.get(CMDBuild.core.constants.Proxy.DATE) < (new Date().valueOf() - timeForEntity) // Compatibility mode with IE older than IE 9 (Date.now())
				);
