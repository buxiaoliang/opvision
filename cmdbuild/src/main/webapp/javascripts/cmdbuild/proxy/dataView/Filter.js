	...
	Ext.define('CMDBuild.proxy.dataView.Filter', {
		...
		getStoreSources: function () {
			return CMDBuild.global.Cache.requestAsStore(CMDBuild.core.constants.Proxy.ENTRY_TYPE, {
				...
				filters: [
					function (record) { // Filters root class and Activity
						var name = record.get(CMDBuild.core.constants.Proxy.NAME);

						return (
							name != CMDBuild.core.constants.Global.getRootNameClasses()
							&& name != CMDBuild.core.constants.Global.getRootNameWorkflows()
						);
					}
				],
				sorters: [
