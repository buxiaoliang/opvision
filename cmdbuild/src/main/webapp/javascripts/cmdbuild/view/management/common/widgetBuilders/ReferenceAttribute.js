(function() {
	...
	function manageCalculatedValues(field, attribute) {
		...
		if (attribute.referencedClassName == "User") {
			model = new CMDBuild.cache.CMReferenceStoreModel({
				Id : MY_USER_ID,
				Description : MY_USER_LABEL
			});
			calculatedValueId = MY_USER_ID;
		} else if (attribute.referencedClassName == "Role") {
			model = new CMDBuild.cache.CMReferenceStoreModel({
				Id : MY_GROUP_ID,
				Description : MY_GROUP_LABEL
			});
			calculatedValueId = MY_GROUP_ID;
		}

		if (!Ext.isEmpty(model)) {
			// Override the ensureToHaveTheValueInStore to deny a getCard() call
			// with -1 as id that throws a not found exception
			...
			// Override the setValue function to change the template with the id
			// of the model (The model id must be a number)
			...
			field.getStore().on('load', function(store, records, successful, eOpts) {
				...
				// Some ExtJS mechanism call the sorting after the the ComboBox
				// List expand() call. So, we have already sort the items to add
				// the new one as first, then override the sort function to do
				// nothing
	...
	CMDBuild.WidgetBuilders.ReferenceAttribute.prototype.buildReadOnlyField = function(attribute) {
		var field = new Ext.form.DisplayField({
			...
			getReadableValue : function() {
				return (this.currentValue) ? this.currentValue.description : null;
			},
			getValue : function() {
				return (this.currentValue) ? this.currentValue.id : null;
			}
		});

		var subFields = getSubFields(attribute, display = true);

		// Overrides setValue function to translate attribute value to
		// description
		var originalSetValue = field.setValue;
		field.setValue = function(value) {
		...
		if (subFields.length > 0) {
			var fieldContainer = {
				xtype : 'container',
				layout : 'hbox',
				items : [ new CMDBuild.field.CMToggleButtonToShowReferenceAttributes({
					subFields : subFields
				}), this.markAsRequired(field, attribute) ]
			};
	...
	function getSubFields(attribute, display) {
		var d = _CMCache.getDomainById(attribute.idDomain), fields = [];
		...
		if (d) {
			...
			for (var i = 0, a = null; i < attrs.length; ++i) {
				...
				if (a.isbasedsp) {
					...
					f.margin = "0 0 0 5";
					if (f) {
						// Mark the sub fields with a flag "cmDomainAttribute"
						// because
						// if a form has one of these fields can know to do a
						// request to
						// populate it. This is needed because the values of the
						// relations attributes
						// are not serialized in the grid data.
						// As an alternative, we can use an event to notify to
						// the controller
						// that the form is ready (has all the fields), so the
						// controller can
						// look for this kind of attributes and make the request
						// if needed
