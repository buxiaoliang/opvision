(function() {
		...
		field.setValue = function(value) {
			if (!Ext.isEmpty(value)) {
				...
				if (value.hasOwnProperty(CMDBuild.core.constants.Proxy.DESCRIPTION)) {
					...
					var id = value[CMDBuild.core.constants.Proxy.ID];
					var foundRecord = store.findRecord('Id', id);
					value = foundRecord.get('Description');
	...
	CMDBuild.WidgetBuilders.LookupAttribute.prototype.genericBuildFieldsetForFilter = function(fieldId, fields, query,
			originalFieldName) {
	...
	function onFakeFieldFocus(attribute) {
		var me = this, pos = this.getPosition();
					...
					buttons : [
							{
								...
								handler : function() {
									var value = fieldForTheWindow.getValue(), rawValue = fieldForTheWindow
											.getRawValue(), recordIndex = me.store.find(me.valueField, value), notInStore = recordIndex == -1, record;
	...
	function buildFieldForTheWindow(attribute) {
		var fieldForTheWindow = CMDBuild.Management.FieldManager.getFieldForAttr(attribute, readOnly = false), fields = fieldForTheWindow.items.items;
