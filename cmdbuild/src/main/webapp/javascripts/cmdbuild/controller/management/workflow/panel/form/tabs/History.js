(function () {
	...
	Ext.define('CMDBuild.controller.management.workflow.panel.form.tabs.History', {
		...
		addCurrentCardToStore: function () {
			...
			selectedEntityValues[CMDBuild.core.constants.Proxy.USER] = this.selectedEntity.get(CMDBuild.core.constants.Proxy.USER);
		...
		buildCurrentEntityModel: function (entityAttributeData) {
			...
			currentEntityModel.set(CMDBuild.core.constants.Proxy.ACTIVITY_NAME, selectedEntityValues['Code']);
		...
		onWorkflowTabHistoryPanelShow: function () {
			if (this.view.isVisible()) {
				...
				if (!Ext.isEmpty(this.selectedEntity) && !Ext.isEmpty(this.selectedEntity.get('IdClass'))) {
					var params = {};
