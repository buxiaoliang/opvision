(function() {
	...
	Ext.define("CMDBuild.controller.management.common.CMCardGridController", {
		...
		//accept a change row every ROWCHANGETIME milliseconds
		timer : undefined,
		...
		onCardSelected: function(selectionModel, selected, eOpts) {
			if (Ext.isArray(selected) && !Ext.isEmpty(selected)) {
				...
				var me = this;
				this.timer = setTimeout(function() {me._onCardSelected(selectionModel, selected, eOpts); }, 100);
			}
		},

		_onCardSelected: function(selectionModel, selected, eOpts) {
			selected = selected[0];
