...
CREATE FUNCTION _graph_get_related_classes("DomainName" character varying, "ClassName" character varying, "CardId" integer, OUT type character varying, OUT total integer) RETURNS SETOF record
    LANGUAGE plpgsql
    AS $_$
BEGIN
RETURN QUERY
...
COMMENT ON FUNCTION _graph_get_related_classes("DomainName" character varying, "ClassName" character varying, "CardId" integer, OUT type character varying, OUT total integer) IS 'TYPE: function';
...
CREATE FUNCTION cm_set_translationuuid() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE TABLE "Building_history" (
    ...
    CONSTRAINT "_Check_Status" CHECK (("Status" = 'U'::bpchar))
)
INHERITS ("Building");
...
CREATE TABLE "User" (
    ...
    "Service" boolean,
    "Privileged" boolean,
    "PasswordExpiration" timestamp without time zone,
    "LastPasswordChange" timestamp without time zone,
    "LastExpiringNotification" timestamp without time zone
)
INHERITS ("Class");
...
CREATE VIEW system_classcatalog AS
 SELECT pg_class.oid AS classid,
    (
        CASE
            WHEN (pg_namespace.nspname = 'public'::name) THEN ''::text
            ELSE ((pg_namespace.nspname)::text || '.'::text)
        END || (pg_class.relname)::text) AS classname,
    pg_description.description AS classcomment,
    (pg_class.relkind = 'v'::"char") AS isview
   FROM ((pg_class
     JOIN pg_description ON (((pg_description.objoid = pg_class.oid) AND (pg_description.objsubid = 0) AND _cm_is_any_class_comment(pg_description.description))))
     JOIN pg_namespace ON ((pg_namespace.oid = pg_class.relnamespace)))
  WHERE (pg_class.reltype > (0)::oid);

CREATE VIEW system_domaincatalog AS
 SELECT pg_class.oid AS domainid,
    "substring"((pg_class.relname)::text, 5) AS domainname,
...
CREATE VIEW system_attributecatalog AS
 SELECT cmtable.classid,
    cmtable.classname,
    pg_attribute.attname AS attributename,
    pg_attribute.attnum AS dbindex,
        CASE
            WHEN (strpos(attribute_description.description, 'MODE: reserved'::text) > 0) THEN '-1'::integer
            ...
            ELSE 0
        END AS attributeindex,
    (pg_attribute.attinhcount = 0) AS attributeislocal,
        CASE pg_type.typname
            WHEN 'geometry'::name THEN (_cm_get_geometry_type(cmtable.classid, (pg_attribute.attname)::text))::name
    ...
    ((notnulljoin.oid IS NOT NULL) OR pg_attribute.attnotnull) AS attributenotnull,
    pg_attrdef.adsrc AS attributedefault,
    attribute_description.description AS attributecomment,
    _cm_attribute_is_unique(cmtable.classid, (pg_attribute.attname)::text) AS isunique,
    _cm_legacy_read_comment(((attribute_description.description)::character varying)::text, ('LOOKUP'::character varying)::text) AS attributelookup,
    _cm_legacy_read_comment(((attribute_description.description)::character varying)::text, ('REFERENCEDOM'::character varying)::text) AS attributereferencedomain,
    _cm_legacy_read_comment(((attribute_description.description)::character varying)::text, ('REFERENCETYPE'::character varying)::text) AS attributereferencetype,
    _cm_legacy_read_comment(((attribute_description.description)::character varying)::text, ('REFERENCEDIRECT'::character varying)::text) AS attributereferencedirect,
        CASE
   ...
   FROM ((((((pg_attribute
     JOIN ( SELECT system_classcatalog.classid,
            system_classcatalog.classname
           FROM system_classcatalog
        UNION
         SELECT system_domaincatalog_1.domainid AS classid,
            system_domaincatalog_1.domainname AS classname
           FROM system_domaincatalog system_domaincatalog_1) cmtable ON ((pg_attribute.attrelid = cmtable.classid)))
...
CREATE VIEW system_inheritcatalog AS
 SELECT pg_inherits.inhparent AS parentid,
    pg_inherits.inhrelid AS childid
   FROM pg_inherits
UNION
 SELECT ('"Class"'::regclass)::oid AS parentid,
    pg_class.oid AS childid
   FROM ((pg_class
  ...
  WHERE ((pg_class.relkind = 'v'::"char") AND (strpos(pg_description.description, 'TYPE: class'::text) > 0));

CREATE VIEW system_privilegescatalog AS
 SELECT DISTINCT ON (permission."IdClass", permission."Code", permission."Description", permission."Status", permission."User", permission."Notes", permission."IdRole", permission."IdGrantedClass") permission."Id",
    permission."IdClass",
    permission."Code",
   ...
   FROM (( SELECT "Grant"."Id",
            ...
            "Grant"."Notes",
            "Grant"."IdRole",
            "Grant"."IdGrantedClass",
            "Grant"."Mode"
           FROM "Grant"
        UNION
         SELECT '-1'::integer,
            '"Grant"'::regclass AS regclass,
            ...
            '-'::character varying AS "varchar"
           FROM system_classcatalog system_classcatalog_1,
            "Role"
          WHERE ((((system_classcatalog_1.classid)::regclass)::oid <> ('"Class"'::regclass)::oid) AND (NOT ((("Role"."Id")::text || ((system_classcatalog_1.classid)::integer)::text) IN ( SELECT (("Grant"."IdRole")::text || ((("Grant"."IdGrantedClass")::oid)::integer)::text)
...
CREATE VIEW system_relationlist AS
 SELECT "Map"."Id" AS id,
    ...
    (_cm_legacy_read_comment(pg_description0.description, 'DESCRDIR'::text))::text AS domaindescription,
    (_cm_legacy_read_comment(pg_description0.description, 'MASTERDETAIL'::text))::text AS domainmasterdetail,
    (_cm_legacy_read_comment(pg_description0.description, 'CARDIN'::text))::text AS domaincardinality,
    (_cm_legacy_read_comment(pg_description2.description, 'DESCR'::text))::text AS classdescription,
    true AS direct,
   ...
   FROM ((((((("Map"
     ...
     LEFT JOIN pg_description pg_description0 ON (((pg_description0.objoid = pg_class0.oid) AND (pg_description0.objsubid = 0) AND _cm_is_domain_comment(pg_description0.description) AND _cm_is_active_comment(pg_description0.description))))
...
UNION
 SELECT "Map"."Id" AS id,
    ...
    "Map"."BeginDate" AS begindate,
    "Map"."Status" AS status,
    (_cm_legacy_read_comment(pg_description0.description, 'DESCRINV'::text))::text AS domaindescription,
    (_cm_legacy_read_comment(pg_description0.description, 'MASTERDETAIL'::text))::text AS domainmasterdetail,
    (_cm_legacy_read_comment(pg_description0.description, 'CARDIN'::text))::text AS domaincardinality,
    (_cm_legacy_read_comment(pg_description1.description, 'DESCR'::text))::text AS classdescription,
    false AS direct,
    NULL::text AS version
   FROM ((((((("Map"
...
CREATE VIEW system_relationlist_history AS
 SELECT "Map"."Id" AS id,
    ...
    NULL::text AS version
   FROM ("Map"
     LEFT JOIN "Class" ON (((("Class"."IdClass")::oid = ("Map"."IdClass2")::oid) AND ("Class"."Id" = "Map"."IdObj2")))),
    pg_class pg_class0,
...
UNION
 SELECT "Map"."Id" AS id,
    ...
    NULL::text AS version
   FROM ("Map"
     LEFT JOIN "Class" ON (((("Class"."IdClass")::oid = ("Map"."IdClass1")::oid) AND ("Class"."Id" = "Map"."IdObj1")))),
    pg_class pg_class0,
    pg_description pg_description0,
    pg_class pg_class1,
    pg_description pg_description1,
...
CREATE VIEW system_treecatalog AS
 SELECT parent_class.classid AS parentid,
   ...
   FROM ((system_inheritcatalog
     JOIN system_classcatalog parent_class ON ((system_inheritcatalog.parentid = parent_class.classid)))
     JOIN system_classcatalog child_class ON ((system_inheritcatalog.childid = child_class.classid)));
...
INSERT INTO "Patch" VALUES (1504, '"Patch"', '2.4.1-01', 'Adds missing "TranslationUuid" values to "LookUp" table and related trigger.', 'A', 'system', '2017-11-23 10:45:56.708581', NULL, NULL);
INSERT INTO "Patch" VALUES (1506, '"Patch"', '2.4.2-01', 'Adds a CHECK on "Status" attribute for historic tables', 'A', 'system', '2017-11-23 10:45:56.762723', NULL, NULL);
INSERT INTO "Patch" VALUES (1508, '"Patch"', '2.4.2-02', 'Fixes the values within "Delay" columns of tables "Email" and "_EmailTemplate"', 'A', 'system', '2017-11-23 10:45:56.771177', NULL, NULL);
INSERT INTO "Patch" VALUES (1510, '"Patch"', '2.4.3-01', 'Creates _graph_get_related_classes function', 'A', 'system', '2017-11-23 10:45:56.775444', NULL, NULL);
INSERT INTO "Patch" VALUES (1512, '"Patch"', '2.5.0-01', 'BIM module functions revision', 'A', 'system', '2017-11-23 10:45:56.783247', NULL, NULL);
INSERT INTO "Patch" VALUES (1514, '"Patch"', '2.5.0-02', 'Add columns to User table for password expiration policy, fix User and Role columns visibility', 'A', 'system', '2017-11-23 10:45:56.806548', NULL, NULL);
INSERT INTO "Patch" VALUES (1516, '"Patch"', '2.5.0-03', 'Create quartz schema', 'A', 'system', '2017-11-23 10:45:56.943025', NULL, NULL);
...
INSERT INTO "User" VALUES (13, '"User"', NULL, 'Administrator', 'A', 'system', '2013-05-09 12:57:49.186365', NULL, 'admin', 'DQdKW32Mlms=', NULL, true, NULL, NULL, NULL, NULL, NULL);
INSERT INTO "User" VALUES (943, '"User"', NULL, 'workflow', 'A', 'admin', '2013-05-09 12:57:49.186365', NULL, 'workflow', 'sLPdlW/0y4msBompb4oRVw==', NULL, true, NULL, NULL, NULL, NULL, NULL);
INSERT INTO "User" VALUES (678, '"User"', NULL, 'Jones Patricia', 'A', 'admin', '2013-05-09 12:57:49.186365', NULL, 'pjones', 'Tms67HRN+qusMUAsM6xIPA==', 'patricia.jones@example.com', true, NULL, NULL, NULL, NULL, NULL);
INSERT INTO "User" VALUES (679, '"User"', NULL, 'Davis Michael', 'A', 'admin', '2013-05-09 12:57:49.186365', NULL, 'mdavis', 'Nlg70IVc7/U=', 'michael.davis@example.com', true, NULL, NULL, NULL, NULL, NULL);
...
SELECT pg_catalog.setval('class_seq', 1516, true);

INSERT INTO quartz.qrtz_locks VALUES ('CMDBuildScheduler', 'STATE_ACCESS');

INSERT INTO quartz.qrtz_scheduler_state VALUES ('CMDBuildScheduler', 'phil1511430307232', 1511430372117, 7500);
...
CREATE UNIQUE INDEX "Report_unique_code" ON "Report" USING btree ((
CASE
    WHEN ((("Code")::text = ''::text) OR (("Status")::text <> 'A'::text)) THEN NULL::text
    ELSE ("Code")::text
END));

CREATE UNIQUE INDEX "_Unique_User_Username" ON "User" USING btree ((
CASE
    WHEN (("Status")::text = 'N'::text) THEN NULL::character varying
    ELSE "Username"
END));

CREATE UNIQUE INDEX "_Unique__EmailAccount_Code" ON "_EmailAccount" USING btree ((
CASE
    WHEN (("Status")::text = 'N'::text) THEN NULL::character varying
    ELSE "Code"
END));

CREATE UNIQUE INDEX "_Unique__EmailTemplate_Code" ON "_EmailTemplate" USING btree ((
CASE
    WHEN (("Status")::text = 'N'::text) THEN NULL::character varying
    ELSE "Code"
END));
...
CREATE UNIQUE INDEX idx_map_accounttemplate_activerows ON "Map_AccountTemplate" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
...
END), (
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::integer
    ELSE "IdObj1"
END), (
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
...
CREATE UNIQUE INDEX idx_map_accounttemplate_uniqueright ON "Map_AccountTemplate" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdObj2"
    ELSE NULL::integer
END));

CREATE UNIQUE INDEX idx_map_assetassignee_activerows ON "Map_AssetAssignee" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_assetassignee_uniqueright ON "Map_AssetAssignee" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_assetreference_activerows ON "Map_AssetReference" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_assetreference_uniqueright ON "Map_AssetReference" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_buildingfloor_activerows ON "Map_BuildingFloor" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_buildingfloor_uniqueright ON "Map_BuildingFloor" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_classemail_activerows ON "Map_ClassEmail" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_classemail_uniqueright ON "Map_ClassEmail" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_classmetadata_activerows ON "Map_ClassMetadata" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_classmetadata_uniqueright ON "Map_ClassMetadata" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_filterrole_activerows ON "Map_FilterRole" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_floorroom_activerows ON "Map_FloorRoom" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_floorroom_uniqueright ON "Map_FloorRoom" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_members_activerows ON "Map_Members" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_members_uniqueright ON "Map_Members" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_networkdeviceconnection_activerows ON "Map_NetworkDeviceConnection" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_officeroom_activerows ON "Map_OfficeRoom" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_officeroom_uniqueright ON "Map_OfficeRoom" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_rfcchangemanager_activerows ON "Map_RFCChangeManager" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_rfcchangemanager_uniqueleft ON "Map_RFCChangeManager" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass1"
    ELSE NULL::regclass
END), (
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdObj1"
    ELSE NULL::integer
END));

CREATE UNIQUE INDEX idx_map_rfcexecutor_activerows ON "Map_RFCExecutor" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_rfcexecutor_uniqueleft ON "Map_RFCExecutor" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass1"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_rfcrequester_activerows ON "Map_RFCRequester" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_rfcrequester_uniqueleft ON "Map_RFCRequester" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass1"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_roomasset_activerows ON "Map_RoomAsset" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_roomasset_uniqueright ON "Map_RoomAsset" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_roomnetworkpoint_activerows ON "Map_RoomNetworkPoint" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_roomnetworkpoint_uniqueright ON "Map_RoomNetworkPoint" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_roomworkplace_activerows ON "Map_RoomWorkplace" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_roomworkplace_uniqueright ON "Map_RoomWorkplace" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_supervisor_activerows ON "Map_Supervisor" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_supervisor_uniqueright ON "Map_Supervisor" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_supplierasset_activerows ON "Map_SupplierAsset" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_supplierasset_uniqueright ON "Map_SupplierAsset" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_suppliercontact_activerows ON "Map_SupplierContact" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_suppliercontact_uniqueright ON "Map_SupplierContact" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_supplierinvoice_activerows ON "Map_SupplierInvoice" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_supplierinvoice_uniqueright ON "Map_SupplierInvoice" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE UNIQUE INDEX idx_map_userrole_activerows ON "Map_UserRole" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_userrole_defaultgroup ON "Map_UserRole" USING btree ((
CASE
...
CREATE UNIQUE INDEX idx_map_workplacecomposition_activerows ON "Map_WorkplaceComposition" USING btree ((
CASE
    WHEN ("Status" = 'N'::bpchar) THEN NULL::regclass
    ELSE "IdDomain"
END), (
...
CREATE UNIQUE INDEX idx_map_workplacecomposition_uniqueright ON "Map_WorkplaceComposition" USING btree ((
CASE
    WHEN (("Status")::text = 'A'::text) THEN "IdClass2"
    ELSE NULL::regclass
END), (
...
CREATE TRIGGER cm_set_translationuuid BEFORE INSERT OR UPDATE ON "LookUp" FOR EACH ROW EXECUTE PROCEDURE cm_set_translationuuid();
