...
public class DataAccessLogicHelper implements SoapLogicHelper {
	...
	private static final List<CMAttribute> EMPTY_ATTRIBUTES = emptyList();
	...
	public ClassSchema getClassSchema(final String name, final boolean includeAttributes) {
		logger.info(marker, "getting schema for class '{}'", name);
		return toClassSchema(dataAccessLogic.findClass(name), includeAttributes);
	}

	public ClassSchema getClassSchema(final long id, final boolean includeAttributes) {
		logger.info(marker, "getting schema for class '{}'", id);
		return toClassSchema(dataAccessLogic.findClass(id), includeAttributes);
	}

	private ClassSchema toClassSchema(final CMClass input, final boolean includeAttributes) {
		final ClassSchema output = new ClassSchema();
		output.setId(input.getId());
		output.setName(input.getIdentifier().getLocalName());
		output.setDescription(input.getDescription());
		output.setSuperClass(input.isSuperclass());
		final List<AttributeSchema> attributes = new ArrayList<>();
		for (final CMAttribute attribute : includeAttributes ? input.getAttributes() : EMPTY_ATTRIBUTES) {
