...
public class JSONDispatcherServiceWrapper implements ClusterAwareCacheable {
	...
	public void clearCache(CacheEvictionPolicy policy) {
		...
		if (policy.clusterPropagation()) {
			ClusterMessage msg = new ClusterMessage(ClusterTarget.JSONDispatcherServiceWrapper.getTargetId(), ClusterEvent.CACHE_EVICT_CACHE);
	...
	public void register(ClusterMessageReceiver receiver) {
		receiver.register(this, ClusterTarget.JSONDispatcherServiceWrapper.getTargetId());
	}
